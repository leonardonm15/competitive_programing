...
void calc(string curr_word, int index, string& word, vector<pair<int, string>>& resp, map<string, string>& mtl) {
    if (index == word.size()) {
        int valor = 1;
...
vector<string> possibilities(const string& word) {
    ...
    string reference_word = word;
    calc(reference_word, 0, reference_word, s, mtl);
